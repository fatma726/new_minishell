...
int			get_exit_status_n(t_node const *node);
void		set_exit_status_n(t_node *node, int status);
bool		is_interactive_n(t_node const *node);
void		set_interactive_n(t_node *node, bool value);
bool		get_nontext_input_n(t_node const *node);
...
char		**ft_setenv_envp(char const *name, char const *value,
				char **envp);
...
/* core utils */
bool		is_blank(char const *s);
long long	ft_atoll(char const *str);

/* process_command */
int			check_operators_at_depth_zero(char const *s, int i,
				int *has_andand);
int			scan_for_oror(char const *s, t_node *n, int *has_andand);
...
char const	*get_error_token(char **args);
...
char const	*check_leading_operators(char **args);
char const	*check_consecutive_ops(char **args, int i);
char const	*check_triple_redir_split(char **args, int i);
char const	*check_invalid_operator_sequences(char **args, int i);
char const	*check_final_token(char **args, int i);
...
int			left_redir_error_public(char const *s, int type, char *tmp);
...
void		handle_absolute_path_error(char **args, char **envp,
